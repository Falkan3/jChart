(function($,window,document,undefined){"use strict";const pluginName="jChart",pluginNameLower=pluginName.toLowerCase(),objPrefix='jchart__',defaults={elements:{container:null,body:null,group:null,figure:null,svg:null,segments:[]},data:[],values:{},placeholder:{data:{value:0,color:{normal:'#00a3f2',active:'#00d8f2',},draw:!0,push:!0,order:null,title:'untitled',strokeWidth:3}},appearance:{type:'donut',baseColor:'#ddd',segmentColor:{normal:'#00a3f2',active:'#00d8f2',},baseOffset:0,baseStrokeWidth:1,strokeWidth:3,animated:!0,title:{chartTitle:'',showValue:!0,showPercentage:!0,showSummary:!0,summaryTitle:'',},radius:100/(2*Math.PI),innerCutout:0.75,centerX:21,centerY:21,subType:'circle',gap:1,},callbacks:{onInit(){},onRefresh(){},onSegmentMouseover(){},onSegmentMouseout(){}}};function Plugin(element,options){this.element=element;this.$element=$(element);this.settings=$.extend(!0,{},defaults,options);this._defaults=defaults;this._name=pluginName;this._nameLower=pluginNameLower;this._objPrefix=objPrefix;this._methods=methods;this._methods.init(this)}
    const methods={init(instance){instance._methods.initElement(instance);if(instance.settings.callbacks.onInit&&$.isFunction(instance.settings.callbacks.onInit)){instance.settings.callbacks.onInit.call(instance)}},initElement(instance){instance._methods.calculateDataValues(instance);instance._methods.initHtml(instance)},calculateDataValues(instance){const values={maxval:0,};const data=instance.settings.data;for(const segment in data){if(data.hasOwnProperty(segment)){data[segment]=$.extend(!0,{},instance.settings.placeholder.data,data[segment]);values.maxval+=data[segment].value}}
            ...
            const data=instance.settings.data;const values=instance.settings.values;let graphData=null;let svg=null;let svgElement=null;let segments=[];switch(instance.settings.appearance.type){case 'donut':graphData=instance._methods.drawBodyBaseDonut(instance,{'type':instance.settings.appearance.subType});svg=graphData.svg;segments=instance._methods.drawBodySegmentDonut(instance,data,values,{'type':instance.settings.appearance.subType});svgElement=instance.settings.elements.body[0].appendChild(svg);svgElement.appendChild(graphData.ring);switch(instance.settings.appearance.type){case 'circle':break;case 'path':svgElement.appendChild(graphData.ring);break}
                ...
                if(instance.settings.appearance.title.showSummary){const drawableSegments=instance._methods.getDrawableSegments(instance,data,!0);if(instance.settings.appearance.title.summaryTitle||drawableSegments.length===1){const chartSummary=instance._methods.drawSvgText(instance,{'class':`${instance._objPrefix}summary`});instance.settings.elements.summary=chartSummary;svgElement.appendChild(chartSummary);if(instance.settings.appearance.title.summaryTitle){instance.settings.elements.summary.innerHTML=instance.settings.appearance.title.summaryTitle}else{instance.settings.elements.summary.innerHTML=`${drawableSegments[0].percentage}%`}}}
            ...
            if(typeof data[segment].color.active==='undefined'){data[segment].color.active=instance.settings.appearance.segmentColor.active}
            const cmd=['M',startX,startY,'A',doughnutRadius,doughnutRadius,0,largeArc,1,endX,endY,'L',startX2,startY2,'A',cutoutRadius,cutoutRadius,0,largeArc,0,endX2,endY2,'Z'];let svgPathOptions={};let drawOnly=!1;let element=null;const titlePartials={'segmentTitle':data[segment].title,'segmentValue':instance._methods.numberFormat(data[segment].value,0,',','\xa0'),'segmentPercentage':Math.round(data[segment].percentage*10)/10};let title=titlePartials.segmentTitle;if(instance.settings.appearance.title.showValue&&instance.settings.appearance.title.showPercentage){title+=`: ${titlePartials.segmentValue} (${titlePartials.segmentPercentage}%)`}else if(instance.settings.appearance.title.showValue){title+=`: ${titlePartials.segmentValue}`}else if(instance.settings.appearance.title.showPercentage){title+=`: ${titlePartials.segmentPercentage}`}
            ...
            if(typeof data[segment].color.active==='undefined'){data[segment].color.active=instance.settings.appearance.segmentColor.active}
            const startCoordinates=instance._methods.getCoordinatesForPercent(base_offset+offset);offset+=data[segment].percentage_raw*settings.modifier;const endCoordinates=instance._methods.getCoordinatesForPercent(offset);const startX=startCoordinates.x;const startY=startCoordinates.y;const endX=endCoordinates.x;const endY=endCoordinates.y;const largeArcFlag=data[segment].percentage_raw*settings.modifier>.5?1:0;const pathData=[`M ${startX} ${startY}`,`A 1 1 0 ${largeArcFlag} 1 ${endX} ${endY}`,`L 0 0`,].join(' ');let svgPathOptions={};let drawOnly=!1;let element=null;if(settings.updateOnly){drawOnly=!0;element=data[segment].element[0];svgPathOptions={fill:data[segment].color.normal,d:pathData}}else{svgPathOptions={'d-id':segment,class:`${instance._objPrefix}pie__segment`,fill:data[segment].color.normal,d:pathData}}
            ...
            return nPath},drawSvgText(instance,options,updateOnly=!1,element=null){const defaults={'class':'','x':instance.settings.appearance.centerX,'y':instance.settings.appearance.centerY,'alignment-baseline':'middle','text-anchor':'middle'};const settings=$.extend(!0,{},defaults,options);let nElement=element;if(!updateOnly){nElement=document.createElementNS('http://www.w3.org/2000/svg','text')}
