(function($,window,document,undefined){"use strict";const pluginName="jChart",pluginNameLower=pluginName.toLowerCase(),objPrefix='jchart--',defaults={elements:{container:null,body:null,group:null,figure:null,svg:null,segments:[]},data:[],values:{},placeholder:{data:{value:0,color:{normal:'#00a3f2',active:'#00d8f2',},draw:!0,push:!0,order:null,title:'untitled',strokeWidth:3}},appearance:{type:'donut',baseColor:'#ddd',segmentColor:{normal:'#00a3f2',active:'#00d8f2',},baseOffset:0,baseStrokeWidth:1,strokeWidth:3,animated:!0,radius:100/(2*Math.PI),innerCutout:0.75,centerX:21,centerY:21,subType:'circle',gap:1,},callbacks:{onInit(){},onRefresh(){},onSegmentMouseover(){},onSegmentMouseout(){}}};function Plugin(element,options){this.element=element;this.$element=$(element);this.settings=$.extend(!0,{},defaults,options);this._defaults=defaults;this._name=pluginName;this._nameLower=pluginNameLower;this._objPrefix=objPrefix;this._methods=methods;this._methods.init(this)}
    const methods={init(instance){instance._methods.initElement(instance);if(instance.settings.callbacks.onInit&&$.isFunction(instance.settings.callbacks.onInit)){instance.settings.callbacks.onInit.call(instance)}},initElement(instance){instance._methods.calculateDataValues(instance);instance._methods.initHtml(instance)},calculateDataValues(instance){const values={maxval:0,};const data=instance.settings.data;for(const segment in data){if(data.hasOwnProperty(segment)){data[segment]=$.extend(!0,{},instance.settings.placeholder.data,data[segment]);values.maxval+=data[segment].value}}
            ...
            const $container=$('<div>',{'class':pluginNameLower});const $figure=$('<figure>');instance.settings.elements.container=$container;instance.settings.elements.figure=$figure;instance.$element.append($container);$container.append($figure)},drawBody(instance){if(typeof instance.settings.elements.body!=='undefined'&&instance.settings.elements.body!==null){instance.settings.elements.body.remove()}
            ...
            const data=instance.settings.data;const values=instance.settings.values;let graphData=null;let svg=null;let svgElement=null;let segments=[];switch(instance.settings.appearance.type){case 'donut':graphData=instance._methods.drawBodyBaseDonut(instance,{'type':instance.settings.appearance.subType});svg=graphData.svg;segments=instance._methods.drawBodySegmentDonut(instance,data,values,{'type':instance.settings.appearance.subType});svgElement=instance.settings.elements.body[0].appendChild(svg);svgElement.appendChild(graphData.ring);switch(instance.settings.appearance.type){case 'circle':break;case 'path':svgElement.appendChild(graphData.ring);break}
                ...
                break;default:break}
            const group=svgElement.appendChild(instance._methods.drawGroup(instance));for(const item in data){if(data.hasOwnProperty(item)){const segment=data[item].element;const segmentElement=group.appendChild(segment);const $segmentElement=$(segmentElement);data[item].element=$segmentElement;instance.settings.elements.segments.push($segmentElement)}}
            instance.settings.elements.svg=svgElement;instance.settings.elements.group=$(group)},addEventListeners(instance){const items=instance.settings.data;for(const item in items){if(items.hasOwnProperty(item)){const segment=items[item].element;segment.on('mouseover',function(){const $this=$(this);const dId=$this.attr('d-id');$this.removeClass('active');$this.addClass('active');switch(instance.settings.appearance.type){case 'donut':switch(instance.settings.appearance.subType){case 'circle':$this.css('stroke',instance.settings.data[dId].color.active);break;case 'path':$this.css('fill',instance.settings.data[dId].color.active);break}
            break;case 'pie':$this.css('fill',instance.settings.data[dId].color.active);break}
            ...
            break;case 'pie':$this.css('fill','');break}
            if(instance.settings.callbacks.onSegmentMouseout&&$.isFunction(instance.settings.callbacks.onSegmentMouseout)){instance.settings.callbacks.onSegmentMouseout.call(instance,instance.settings.data[dId],instance.settings.data[dId])}})}}},drawBodyBaseDonut(instance,options){const defaults={type:'circle',};const settings=$.extend(!0,{},defaults,options);let response={};const svg=document.createElementNS("http://www.w3.org/2000/svg","svg");svg.setAttribute('class',instance._objPrefix+'donut');svg.setAttribute('width','100%');svg.setAttribute('height','100%');svg.setAttribute('viewBox',`0 0 ${instance.settings.appearance.centerX * 2} ${instance.settings.appearance.centerY * 2}`);svg.setAttributeNS("http://www.w3.org/2000/xmlns/","xmlns:xlink","http://www.w3.org/1999/xlink");let donutRing;switch(settings.type){case 'circle':donutRing=instance._methods.drawSvgCircle(instance,{class:instance._objPrefix+'donut--ring'+instance._objPrefix+'donut--ring-circle',fill:'transparent',stroke:instance.settings.appearance.baseColor,'stroke-width':instance.settings.appearance.baseStrokeWidth});const donutHole=null;response={'svg':svg,'ring':donutRing,'hole':donutHole};break;case 'path':const baseDoughnutRadius=instance.settings.appearance.radius+instance.settings.appearance.baseStrokeWidth;const cutoutRadius=instance.settings.appearance.radius*(instance.settings.appearance.innerCutout);const baseCutoutRadius=cutoutRadius-instance.settings.appearance.baseStrokeWidth;const startRadius=-Math.PI/2,segmentAngle=(99.9999/100)*(Math.PI*2),endRadius=startRadius+segmentAngle,startX=instance.settings.appearance.centerX+Math.cos(startRadius)*baseDoughnutRadius,startY=instance.settings.appearance.centerY+Math.sin(startRadius)*baseDoughnutRadius,endX2=instance.settings.appearance.centerX+Math.cos(startRadius)*baseCutoutRadius,endY2=instance.settings.appearance.centerY+Math.sin(startRadius)*baseCutoutRadius,endX=instance.settings.appearance.centerX+Math.cos(endRadius)*baseDoughnutRadius,endY=instance.settings.appearance.centerY+Math.sin(endRadius)*baseDoughnutRadius,startX2=instance.settings.appearance.centerX+Math.cos(endRadius)*baseCutoutRadius,startY2=instance.settings.appearance.centerY+Math.sin(endRadius)*baseCutoutRadius;const cmd=['M',startX,startY,'A',baseDoughnutRadius,baseDoughnutRadius,0,1,1,endX,endY,'L',startX2,startY2,'A',baseCutoutRadius,baseCutoutRadius,0,1,0,endX2,endY2,'Z'];donutRing=instance._methods.drawSvgPath(instance,{class:instance._objPrefix+'donut--ring'+instance._objPrefix+'donut--ring-path',fill:instance.settings.appearance.baseColor,d:cmd.join(' '),});response={'svg':svg,'ring':donutRing};break}
            return response},drawBodySegmentDonut(instance,data,values,options){const defaults={type:'circle',updateOnly:!1,modifier:1,};const settings=$.extend(!0,{},defaults,options);let segments=[];if(settings.updateOnly){data=instance.settings.data;segments=instance.settings.data}
            const gap=instance.settings.appearance.gap;switch(settings.type){case 'circle':const base_offset=25;let offset=0;for(const segment in data){if(data.hasOwnProperty(segment)){const local_offset=(100-data[segment].percentage*settings.modifier);if(data[segment].draw===!0){if(typeof data[segment].color.normal==='undefined'){data[segment].color.normal=instance.settings.appearance.segmentColor.normal}
                if(typeof data[segment].color.active==='undefined'){data[segment].color.active=instance.settings.appearance.segmentColor.active}
                let svgCircleOptions={};let drawOnly=!1;let element=null;if(settings.updateOnly){drawOnly=!0;element=data[segment].element[0];svgCircleOptions={'d-id':segment,class:instance._objPrefix+'donut--segment'+' '+instance._objPrefix+'donut--segment-circle',title:data[segment].title+': '+data[segment].value+' ('+data[segment].percentage+'%)',fill:'transparent',stroke:data[segment].color.normal,'stroke-width':data[segment].strokeWidth,'stroke-dasharray':(data[segment].percentage*settings.modifier-gap)+' '+(local_offset+gap),'stroke-dashoffset':base_offset+offset}}else{svgCircleOptions={title:data[segment].title+': '+data[segment].value+' ('+data[segment].percentage+'%)',fill:'transparent',stroke:data[segment].color.normal,'stroke-width':data[segment].strokeWidth,'stroke-dasharray':(data[segment].percentage*settings.modifier-gap)+' '+(local_offset+gap),'stroke-dashoffset':base_offset+offset}}
                ...
                break;case 'path':let startRadius=-Math.PI/2;const doughnutRadius=instance.settings.appearance.radius;const cutoutRadius=doughnutRadius*(instance.settings.appearance.innerCutout);const centerX=instance.settings.appearance.centerX;const centerY=instance.settings.appearance.centerY;for(const segment in data){if(data.hasOwnProperty(segment)){const gapPercent=(gap/100),gapAngle=gapPercent*(Math.PI*2),segmentAngle=settings.modifier*((data[segment].percentage_raw-gapPercent)*(Math.PI*2)),endRadius=startRadius+segmentAngle,largeArc=((endRadius-startRadius)%(Math.PI*2))>Math.PI?1:0,startX=centerX+Math.cos(startRadius)*doughnutRadius,startY=centerY+Math.sin(startRadius)*doughnutRadius,endX2=centerX+Math.cos(startRadius)*cutoutRadius,endY2=centerY+Math.sin(startRadius)*cutoutRadius,endX=centerX+Math.cos(endRadius)*doughnutRadius,endY=centerY+Math.sin(endRadius)*doughnutRadius,startX2=centerX+Math.cos(endRadius)*cutoutRadius,startY2=centerY+Math.sin(endRadius)*cutoutRadius;if(data[segment].draw===!0){if(typeof data[segment].color.normal==='undefined'){data[segment].color.normal=instance.settings.appearance.segmentColor.normal}
                if(typeof data[segment].color.active==='undefined'){data[segment].color.active=instance.settings.appearance.segmentColor.active}
                const cmd=['M',startX,startY,'A',doughnutRadius,doughnutRadius,0,largeArc,1,endX,endY,'L',startX2,startY2,'A',cutoutRadius,cutoutRadius,0,largeArc,0,endX2,endY2,'Z'];let svgPathOptions={};let drawOnly=!1;let element=null;if(settings.updateOnly){drawOnly=!0;element=data[segment].element[0];svgPathOptions={'d-id':segment,class:instance._objPrefix+'donut--segment'+' '+instance._objPrefix+'donut--segment-path',title:data[segment].title+': '+data[segment].value+' ('+data[segment].percentage+'%)',fill:data[segment].color.normal,stroke:'transparent','stroke-width':data[segment].strokeWidth,d:cmd.join(' ')}}else{svgPathOptions={title:data[segment].title+': '+data[segment].value+' ('+data[segment].percentage+'%)',fill:data[segment].color.normal,stroke:'transparent','stroke-width':data[segment].strokeWidth,d:cmd.join(' ')}}
                ...
                break}
            return segments},drawBodyBasePie(instance){const svg=document.createElementNS("http://www.w3.org/2000/svg","svg");svg.setAttribute('class',instance._objPrefix+'pie');svg.setAttribute('width','100%');svg.setAttribute('height','100%');svg.setAttribute('viewBox','-1 -1 2 2');svg.setAttribute('style','transform: rotate(-0.25turn)');svg.setAttributeNS("http://www.w3.org/2000/xmlns/","xmlns:xlink","http://www.w3.org/1999/xlink");return{'svg':svg}},drawBodySegmentPie(instance,data,values,options){const defaults={updateOnly:!1,modifier:1,};const settings=$.extend(!0,{},defaults,options);let segments=[];const base_offset=0;let offset=0;for(const segment in data){if(data.hasOwnProperty(segment)){if(data[segment].draw===!0){if(typeof data[segment].color.normal==='undefined'){data[segment].color.normal=instance.settings.appearance.segmentColor.normal}
            if(typeof data[segment].color.active==='undefined'){data[segment].color.active=instance.settings.appearance.segmentColor.active}
            const startCoordinates=instance._methods.getCoordinatesForPercent(base_offset+offset);offset+=data[segment].percentage_raw*settings.modifier;const endCoordinates=instance._methods.getCoordinatesForPercent(offset);const startX=startCoordinates.x;const startY=startCoordinates.y;const endX=endCoordinates.x;const endY=endCoordinates.y;const largeArcFlag=data[segment].percentage_raw*settings.modifier>.5?1:0;const pathData=[`M ${startX} ${startY}`,`A 1 1 0 ${largeArcFlag} 1 ${endX} ${endY}`,`L 0 0`,].join(' ');let svgPathOptions={};let drawOnly=!1;let element=null;if(settings.updateOnly){drawOnly=!0;element=data[segment].element[0];svgPathOptions={fill:data[segment].color.normal,d:pathData}}else{svgPathOptions={'d-id':segment,class:instance._objPrefix+'pie--segment',fill:data[segment].color.normal,d:pathData}}
            const pieSegment=instance._methods.drawSvgPath(instance,svgPathOptions,drawOnly,element);if(!settings.updateOnly){data[segment].element=pieSegment;segments.push(pieSegment)}}else{if(data[segment].push===!0){offset+=data[segment].percentage_raw}}}}
            ...
            return nCircle},drawSvgPath(instance,options,updateOnly=!1,element=null){const defaults={'class':'','fill':'#fff','d':'','stroke':'','stroke-width':0,'stroke-dasharray':'','stroke-dashoffset':0,};const settings=$.extend(!0,{},defaults,options);let nPath=element;if(!updateOnly){nPath=document.createElementNS('http://www.w3.org/2000/svg','path')}
            ...
            return nPath},getCoordinatesForPercent(percent){const x=Math.cos(2*Math.PI*percent);const y=Math.sin(2*Math.PI*percent);return{x:x,y:y}},lightenRgbColors(c,n){let d;for(let i=3;i--;c[i]=d<0?0:d>255?255:d|0){d=c[i]+n}
            ...
            progress=easeOut(progress);if(progress>1)
            ...
            return-1})}
            return output},};$.fn[pluginName]=function(options){let instances=[];this.each(function(){if(!$.data(this,"plugin_"+pluginName)){const instance=new Plugin(this,options);$.data(this,"plugin_"+pluginName,instance);instances.push(instance)}});if(instances.length===1){return instances[0]}
